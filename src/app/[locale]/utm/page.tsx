...
import CampaignLinksStudio from "@/components/CampaignLinksStudio";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "utm" });

  return {
    title: t("meta_title"),
    description: t("meta_description"),
    alternates: {
      canonical: `${SITE_URL}${localizedPath(locale, "utm")}`,
      languages: languageAlternates("utm"),
    },
    openGraph: {
      title: t("meta_title"),
      description: t("meta_description"),
      url: `${SITE_URL}${localizedPath(locale, "utm")}`,
    },
  };
}

export default async function UtmPage({ params }: Props) {
  const { locale: localeValue } = await params;
  const locale: Locale = isLocale(localeValue) ? localeValue : "pt";
